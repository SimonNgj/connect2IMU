...
public class mainCS : MonoBehaviour
{
    ...
    void Update ()
    {
        ...
        if (sp.IsOpen)
        {
            try
            {
                ...
                int rot0, rot1, rot2, rot3, roll, pitch, yaw;
                int.TryParse(pos[0], out rot0);
                int.TryParse(pos[1], out rot1);
                int.TryParse(pos[2], out rot2);
                int.TryParse(pos[3], out rot3);
                int.TryParse(pos[4], out roll);
                int.TryParse(pos[5], out pitch);
                int.TryParse(pos[6], out yaw);
    ...
    void RotateObject(int obj0, int obj1, int obj2)
    {
        Lhand.transform.localRotation = Quaternion.RotateTowards(Lhand.transform.localRotation, Quaternion.Euler(-45 + 1.0f * obj0, 90 + 1.0f * obj1, 180 + 1.0f*obj2), 20f);
    }
}
